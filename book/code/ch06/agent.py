"""
MiniAgent — Chapter 6: 上下文管理
Agent 现在能管理有限的上下文窗口，处理任意长度的任务。

用法 (交互模式):
    python miniagent/agent.py

用法 (单次执行):
    python miniagent/agent.py "重构整个项目并运行测试"
"""
...
def agent_loop(
    ...
    # NEW: compact 工具需要 messages 和 client，使用 lambda 延迟绑定
