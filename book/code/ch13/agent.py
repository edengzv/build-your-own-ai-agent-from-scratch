"""
MiniAgent — Chapter 13: 工作隔离
多个 Agent 在各自的 Git Worktree 中工作，互不干扰。
...
TOOLS = [
    ...
    *TEAM_TOOLS,
    *LEAD_PROTOCOL_TOOLS,
    *WORKTREE_TOOLS,  # NEW: worktree_create, worktree_remove, worktree_list
]
...
def _summarize_input(block) -> str:
    ...
    elif block.name in ("worktree_create", "worktree_remove"):  # NEW
        return inp.get("task_id", "")
    return str(inp)[:80]
...
def agent_loop(
    ...
    wt_manager: WorktreeManager | None = None,  # NEW
) -> None:
...
def repl():
    ...
    while True:
        ...
        agent_loop(messages, todo_manager, context_manager, bg_manager, task_graph, team_manager, wt_manager)
