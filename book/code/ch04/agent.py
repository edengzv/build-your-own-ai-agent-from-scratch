"""
MiniAgent — Chapter 4: 任务规划
Agent 现在能通过 TodoManager 规划和追踪多步骤任务。

用法 (交互模式):
    python miniagent/agent.py

用法 (单次执行):
    python miniagent/agent.py "创建一个 Python 包，包含 setup.py 和 README"
"""
...
# --- Agent 核心循环 ---
def agent_loop(messages: list, todo_manager: TodoManager | None = None) -> None:  # CHANGED
    ...
    while True:
        ...
        for block in response.content:
            if block.type == "tool_use":
                handler = handlers.get(block.name)  # CHANGED: 使用局部 handlers
                if handler is None:
...
def repl():
    """交互式对话循环。"""
    messages = []
    todo_manager = TodoManager()  # NEW: 创建任务管理器
    print("MiniAgent REPL (输入 'exit' 退出, 'clear' 清空对话)")
    print(f"工作目录: {WORKSPACE}")
    print(f"工具: bash, read_file, write_file, edit_file, todo")  # CHANGED
    print("-" * 50)
    ...
    while True:
        ...
        if user_input.lower() == "clear":
            messages.clear()
            todo_manager.__init__()  # 重置任务列表
            print("[对话和任务已清空]")
            continue

        messages.append({"role": "user", "content": user_input})
        agent_loop(messages, todo_manager)  # CHANGED: 传入 todo_manager
