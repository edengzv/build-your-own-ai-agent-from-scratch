...
用法 (单次执行):
    python miniagent/agent.py "用 code-review 技能审查 agent.py"
"""
...
def _summarize_input(block) -> str:
    ...
    elif block.name == "load_skill":  # NEW
        return inp.get("name", "")
    return str(inp)[:80]


# --- Agent 核心循环 ---
def agent_loop(messages: list, todo_manager: TodoManager | None = None) -> None:
