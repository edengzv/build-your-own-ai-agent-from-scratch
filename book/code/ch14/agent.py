"""
MiniAgent — Chapter 14: 安全与权限
Agent 现在有安全层：路径沙箱增强、命令分级、危险操作确认、工具权限模型。
...
def safe_path(path: str) -> str:
    """确保路径在工作目录内，防止路径遍历攻击。"""
    return _sandbox.check_path(path)  # CHANGED: 使用 Sandbox
...
def _summarize_input(block) -> str:
    ...
    elif block.name in ("claim_task", "complete_my_task"):
        return inp.get("task_id", "")
    elif block.name in ("worktree_create", "worktree_remove"):
        return inp.get("task_id", "")
    return str(inp)[:80]
...
def agent_loop(
    ...
    sandbox: Sandbox | None = None,  # NEW
    gate: ConfirmationGate | None = None,  # NEW
) -> None:
...
def repl():
    ...
    while True:
        ...
        agent_loop(messages, todo_manager, context_manager, bg_manager,
                   task_graph, team_manager, wt_manager, sandbox, gate)  # CHANGED
...
def main():
    if len(sys.argv) >= 2:
        ...
        agent_loop(messages, todo_manager, context_manager, bg_manager,
                   task_graph, team_manager, wt_manager, _sandbox, _gate)  # CHANGED
    else:
        repl()
