...
def _build_chapter_opener(meta: dict, images_dir: Path) -> str:
    ...
    # Include chapter number in h1 text so PDF bookmarks/TOC show numbered chapters
    escaped_title = html_mod.escape(meta["title"])
    ch_num = meta["number"]
    parts.append(
        f'<h1 id="{aid}" style="bookmark-label: \'第 {ch_num} 章　{escaped_title}\'">'
        f'{escaped_title}</h1>'
    )
    ...
    for ch_file in chapters:
        ...
        for line in ch_file.read_text(encoding="utf-8").split("\n"):
            ...
            if line.startswith("# "):
                ...
                # Format "Chapter N: Title" as "第 N 章  Title" for Chinese ToC
                m_ch = re.match(r"Chapter (\d+): (.+)", title)
                if m_ch:
                    toc_label = f'第 {m_ch.group(1)} 章\u3000{m_ch.group(2)}'
                else:
                    toc_label = title
                items.append(
                    f'<li class="toc-h1">'
                    f'<a href="#{aid}">'
                    f'<span class="toc-text">{html_mod.escape(toc_label)}</span>'
...
def build_front_matter_html(lang: str = "zh") -> str:
    ...
    if lang == "en":
        ...
        halftitle = (
            '<div class="halftitle-page">\n'
            '<p class="halftitle-title">Skill 工程</p>\n'
            '<p class="halftitle-subtitle">写给 AI 时代的技能设计指南</p>\n'
            '</div>'
        )
        titlepage = (
            '<div class="titlepage">\n'
            '<p class="titlepage-title">Skill 工程</p>\n'
            '<p class="titlepage-subtitle">写给 AI 时代的技能设计指南</p>\n'
            '<p class="titlepage-subtitle-en">'
            'The Art of Skill Engineering: A Practical Guide to Crafting AI Agent Skills</p>\n'
            '<hr class="titlepage-rule"/>\n'
            '<p class="titlepage-author">zhengfeng</p>\n'
            '</div>'
        )
        copyright_page = (
            '<div class="copyright-page">\n'
            '<p><strong>Skill 工程：写给 AI 时代的技能设计指南</strong></p>\n'
            '<p>The Art of Skill Engineering: A Practical Guide to Crafting AI Agent Skills</p>\n'
            '<p>&copy; 2025 zhengfeng. All rights reserved.</p>\n'
            '<p class="copyright-meta">First Edition, April 2025</p>\n'
            '<p class="copyright-meta">Typeset with WeasyPrint &amp; CSS Paged Media</p>\n'
...
<head>
<meta charset="utf-8">
<title>Skill 工程：写给 AI 时代的技能设计指南</title>
<style>
/* === Book stylesheet === */
{css_text}
