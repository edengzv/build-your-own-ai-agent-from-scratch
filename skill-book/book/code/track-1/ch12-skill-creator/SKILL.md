...
| Tool | When | Purpose |
|------|------|---------|
| AskUserQuestion | Pass 0-1 | 复杂度确认，需求收集，方案确认 |
...
在收集需求前，快速评估任务复杂度，决定走哪条路径。
简单 Skill 过度工程化会降低可维护性（来自案例分析洞察）。
...
正文内容要求：
- 角色声明（一句话定位，明确专业领域）
- 编号的执行步骤（或 Multi-Pass 工作流，每步只做一件事）
- Tool Usage 表（只列实际需要的工具）
- 输出格式说明（明确、具体、可验证）
- 约束条件（每条附带原因，禁止模糊表述）
- 质量标准（可自检的清单）
...
从 case-insights.md 提取同类型 Skill 的设计经验：
- 该类型 Skill 的典型行数范围
- 常见的有效约束模式（哪些约束被证明有效）
...
验证脚本检查：Frontmatter、name (kebab-case)、description、步骤结构、
输出格式、约束、Tool Usage 表、Guardrails、模糊约束检测、行数。
...
为 description 生成 5 个**正触发**和 5 个**负触发**测试：
...
| 2 | 指令结构 | 五要素完整性（角色/步骤/输出/约束/标准），模式选择合理性 |
...
**维度 1 — 触发设计：**
- description 是否遵循 What+When+Keywords 三步法？
- 长度是否在 50-150 字范围内？
- When 子句是否使用动词短语？
- 是否存在欠触发风险（太短/太技术化）？
...
- 五要素是否齐全？
- 步骤是否编号且每步只做一件事？
- Multi-Pass（如有）是否有暂停点？
- 命令表（如有）是否动词优先、正交？

**维度 3 — 自由度匹配：**
- 窄桥型任务（如 commit message）：约束应严格
- 开阔地型任务（如 code review）：约束应宽松
- 约束是否过松（"合适的"）或过严（限制创造性）？

**维度 4 — 工具编排：**
- Tool Usage 表是否存在且完整？
- 表中工具是否都在指令中实际使用？
- 指令中使用的工具是否都在表中声明？
- Pipeline 逻辑是否合理（无循环依赖、有错误分支）？

**维度 5 — 架构决策：**
- < 80 行是否保持了单文件？
- scripts/ 中的逻辑是否真的是确定性的？
- reference/ 中的知识是否真的需要 Agent 推理？
- 脚本是否遵循 shebang + set -e + stdout JSON + stderr status？

**维度 6 — 质量防线：**
- 是否有 Guardrails（硬性边界）？
- 是否命中 8 种已知反模式？
- 断言（如有）是否覆盖存在性+质量+安全性三层？

**维度 7 — 可维护性：**
- 行数是否在同类型 Skill 的典型范围内？
- 是否保持单一职责（不是万能 Skill）？
- 指令是否清晰易懂，新人能否理解？
...
> 此部分记录 skill-creator 从 v01 到 v12 的渐进演化，
> 每个版本对应一章的核心技巧，展示"一个 Skill 如何从 10 行成长为 500+ 行"。

| 版本 | 章节 | 新增技巧 | 关键变化 |
|------|------|---------|---------|
| v01 | Ch1 基本结构 | Frontmatter + 步骤 | 10 行极简骨架 |
| v02 | Ch2 Description | What+When+Keywords | description 从 5 字增到 100+ 字 |
| v03 | Ch3 五要素 | 角色/步骤/输出/约束/标准 | 结构化指令，约束附 Why |
| v04 | Ch4 工作流 | Multi-Pass 模式 | 3-Pass 流程 + 暂停点 |
| v05 | Ch5 命令 | 命令路由 | create/review/improve 三命令 |
| v06 | Ch6 示例 | I/O 示例 + 反例 | 完整对话锚定 + 反面教材 |
| v07 | Ch7 三层架构 | scripts/ + reference/ | 验证脚本 + 模式文档分离 |
| v08 | Ch8 工具 | Tool Usage 表 | 工具声明 + Pipeline 编排 |
| v09 | Ch9 质量 | Guardrails + 反模式 | 硬边界 + 10 项清单 + 6 种反模式 |
| v10 | Ch10 评测 | 触发测试 + 断言 | Pass 4 + run_eval.sh + 三层断言 |
| v11 | Ch11 案例 | 复杂度路由 + 案例注入 | Pass 0 + 7 维审查 + 12 项清单 |
| v12 | Ch12 终极 | 全技巧整合 + 自指性 | 生产就绪的元 Skill |

> 对比任意两个版本的 diff，可以精确看到每章技巧如何改进 Skill：
> `diff code/track-2/v06-examples/SKILL.md code/track-2/v07-architecture/SKILL.md`
