...
if grep -q "^name:" "$SKILL_PATH"; then
  ...
  if echo "$NAME" | grep -qE "^[a-z][a-z0-9-]*$"; then
    echo "  ✓ name 字段: $NAME (kebab-case)" >&2
    PASS=$((PASS + 1))
  else
...
if [ "$LINES" -gt 300 ]; then
  ...
  WARNINGS="\"line_count_warning\": \"${LINES} lines exceeds recommended 300\","
elif [ "$LINES" -lt 10 ]; then
  echo "  ⚠ 行数过少: ${LINES} 行（可能不完整）" >&2
  WARNINGS="\"line_count_warning\": \"${LINES} lines may be incomplete\","
else
  echo "  ✓ 行数合理: ${LINES} 行" >&2
  PASS=$((PASS + 1))
fi
