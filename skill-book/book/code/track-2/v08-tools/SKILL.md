...
| Read | review/improve | 读取已有 SKILL.md 和 reference/ 文档 |
| Write | Pass 2 完成后 | 保存生成的 SKILL.md 到目标目录 |
...
**工具**：Read（查阅 reference/skill-patterns.md 确定模式）
...
- **Tool Usage 表**：列出 Skill 使用的每个工具、使用时机和目的
- 输出格式说明
- 约束条件（每条附带原因）
- 质量标准
...
4. 约束条件是否都解释了原因？
5. 命令表（如有）是否使用动词优先命名？
6. Tool Usage 表是否完整且准确？
7. 三层架构：确定性逻辑是否提取到 scripts/？知识在 reference/？
8. 总行数是否在合理范围内？
...
| 工具使用 | 是否有 Tool Usage 表？工具选择是否合理？ |
...
2. 针对每个 Fail 项生成修改方案
3. 展示修改方案，请用户确认
4. 用 Edit 工具应用局部修改（而非全量重写），输出改进后的 SKILL.md
...
**为什么差**：description 过短无法触发；无结构、无 Tool Usage、无约束、"合适的"是模糊表述。
...
- Tool Usage 表必须列出每个工具的使用阶段和目的，因为这让审查者能追踪工具依赖
...
- Tool Usage 表与实际指令中的工具调用一致（不多不少）
